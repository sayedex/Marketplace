...
export const Swap = ({ id }: Props) => {
  ...
  const contract = router.query.id as string;
   const { rcvToken ,TokenList,price,mintTokenBalance,SoldTokenBalance} = useAppSelector((state) => state.pools);
  ...
  return (
    <div className='relative flex flex-col gap-5 w-full lg:w-3/6   '>
      ...
      <div className='flex flex-row gap-x-5'>
        ...
        <Box name={rcvToken.symbol} value={SoldTokenBalance} type='Balance' />
      </div>
      {/* content */}
